...
export class Football extends Element3D implements ElementType {
  flatHeight: number = 0;
  legsHeight: number = 0;
  ...
  // Model taken from https://codepen.io/tristone_playground/pen/QgdabN

  public CreateElement() {
    var sideLength, a, b, c, d, e, f, g, h, lineWidth;
    ...
    for (var i = 0; i < facesCordsPent.length; ++i) {
      var faceCord = facesCordsPent[i];
    ...
    var facesGeomPent = new THREE.PolyhedronGeometry(
      _.flattenDeep(cords),
      indicesOfFaces,
      radius,
      2
    ...
    football.add(new THREE.Mesh(facesGeomPent, material));
    ...
    for (var i = 0; i < facesCordsHex.length; ++i) {
      var faceCord = facesCordsHex[i];
      for (var j = 0; j < faceCord.length - 2; ++j) {
        indicesOfFaces.push(faceCord[0]);
        indicesOfFaces.push(faceCord[j + 1]);
        indicesOfFaces.push(faceCord[j + 2]);
    ...
    var facesGeomHex = new THREE.PolyhedronGeometry(
      _.flattenDeep(cords),
      indicesOfFaces,
      radius,
      2
    ...
    const hexMaterial = this.vpt.material.clone();
    hexMaterial.color = new THREE.Color(0xffffff);

    football.add(new THREE.Mesh(facesGeomHex, hexMaterial));

    football.position.set(0, radius * 0.002, 0);
    football.scale.set(0.002, 0.002, 0.002);

    this.element.add(football);
  }
}
